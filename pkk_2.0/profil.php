...
$query = "SELECT id_user, name, email, password, phone, address, saldo, foto FROM users WHERE name = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['foto']['name'])) {
        $profile_picture = $_FILES['foto']['name'];
        $target_dir = "uploads/";
        $target_file = $target_dir . basename($profile_picture);

        if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
            $updateQuery = "UPDATE users SET name=?, email=?, phone=?, foto=? WHERE name=?";
        ...
        $updateQuery = "UPDATE users SET name=?, email=?, phone=? WHERE name=?";
        $stmt = $conn->prepare($updateQuery);
